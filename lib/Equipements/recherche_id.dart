...
class Recherche extends StatefulWidget {
  final docsid;
  const Recherche({Key? key, this.docsid}) : super(key: key);
  @override
...
torecherche (String quary) async {
 ...
 addData() async {
  for (var element in data) {
    FirebaseFirestore.instance.collection("bd_equip").add(element);
  }
  ...
  Widget build(BuildContext context) {
    ...
    appBar: AppBar(
      ...
      body: StreamBuilder<QuerySnapshot> (
        stream: FirebaseFirestore.instance.collection("bd_equip").snapshots(),
              ...
              if (rech.isEmpty){
                 return Card(
                          ...
                          subtitle: Column(
                            ...
                            children: [
                              Text("Désignation : ${snapshots.data!.docs[i]["Désignation"]}"),
                              Text("Marque : ${snapshots.data!.docs[i]["Marque"]}"),
                                   ...
                                   SizedBox(
                                    ...
                                    child: Row(
                                      children: [
                                        IconButton(
                                          onPressed: () {
                                            Navigator.of(context)
                                              ...
                                              return Edit(BD: snapshots.data!.docs[i], docsid: snapshots.data!.docs[i].id);
                                            }));
                                          },
                                        ...
                                        IconButton(
                                          onPressed: () async {
                                            equipref.doc(snapshots.data!.docs[i].id).delete();
                                           setState(() {
                                             equipref.get();
                                           });
                                            showDialog(
